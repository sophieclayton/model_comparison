...
dUNbiomass=zeros(3060,510);
dVNbiomass=zeros(3060,510);

%for t=1:365;
    %day=t;
    
    inu='/scratch/sclayton/high_res/UVEL_1999.data';
    ufid=fopen(inu,'r','ieee-be');
    
    inv='/scratch/sclayton/high_res/VVEL_1999.data';
    vfid=fopen(inv,'r','ieee-be');
    
    inp='/scratch/sclayton/high_res/Phyto/AveSurfPhyto.1999.data';
    ...
    for k=1;
        ...
        dVNbiomass=dVNbiomass+dVN;
        
        clear divVN
        
        % write out the flux divergence of the tracer
        out=sprintf('/scratch/sclayton/high_res/horz_flux/phy%02d/ANNAVE.divUNtop.1999.data',n);
        fidout=fopen(out,'w','b');
        fwrite(fidout,dUN,'real*4');
        fclose(fidout);
        
        out=sprintf('/scratch/sclayton/high_res/horz_flux/phy%02d/ANNAVEdivVNtop.1999.data',n);
        fidout=fopen(out,'w','b');
