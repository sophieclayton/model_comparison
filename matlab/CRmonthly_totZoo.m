...
indir='/scratch/sclayton/coarse_rerun2/extract/NutsZoo/';
...
for m=1:12;
    ...
    for day=startday(m):1:endday(m);
        in=sprintf('%sIntNutsZoo.1999.%04d.data',indir,day);
        tmp=readbin(in,[360 160 21]);
        tmpp=tmp(:,:,8)+tmp(:,:,12);clear tmp;tmp=tmpp;clear tmpp
        tmp(HFCR==0)=NaN;
...
out=sprintf('%sIntZoo.monthly.1999.data',outdir);
