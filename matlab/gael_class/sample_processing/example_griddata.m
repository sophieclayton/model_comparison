...
fprintf('note that griddata puts values in remote places ... \n');
fprintf('so one should obviously be cautious when using this routine\n');
%msk=mygrid.hFacC(:,:,1); z_interp(find(msk==0))=NaN;

%%%%%%%%%%%%%%%%%%%%%%%
%illustrate the result:

figure; set(gcf,'Units','Normalized','Position',[0.1 0.1 0.4 0.6]);
x=[lon-360;lon]; y=[lat;lat]; z=[etan;etan];
pcolor(x,y,z); axis([-180 180 -90 90]); shading flat; caxis([-2 1]); colorbar;

figure; set(gcf,'Units','Normalized','Position',[0.5 0.1 0.4 0.6]);
