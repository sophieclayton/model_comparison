...
if ~isempty(choiceV3orV4)&isempty(whos('mygrid'));
  gcmfaces_path;
  dir0='/net/altix3700/raid4/gforget/mysetups/ecco_v4/RUNS/matlab_class_material/';
  dirGrid=[dir0 'GRID' choiceV3orV4  '/'];
  if strcmp(choiceV3orV4,'v4'); nF=5; else; nF=1; end;
  global mygrid; mygrid=[]; grid_load(dirGrid,nF);
end;

dirOut=dirFld; %dirOut='/net/altix3700/raid4/gforget/mysetups/ecco_v4/RUNS/matlab_class/TMP/';
...
%set up directories:
dirCUR=pwd;
eval(['!mkdir ' dirOut 'movies/']);
eval(['!mkdir ' dirOut 'movies/TMP/']);
eval(['cd ' dirOut 'movies/TMP/']);
...
%fld=convert2array(NaN*zeros(n1,n2,nt),mygrid.XC);
fld=NaN*mygrid.XC;
for tt=1:nt;
fld(:,:,tt)=v4_read_bin([dirFld fileList(tt).name],1,k);
end;
...
eval(['!convert tmp_' nameFld '_' num2str(k) '_' tt_txt '.tiff tmp_' nameFld '_' num2str(k) '_' tt_txt '.gif']);
end;
eval(['!gifsicle --delay=20 --loop tmp_' nameFld '_' num2str(k) '_*.gif  > ' nameFld '_' num2str(k) '.gif']);
eval(['!\rm tmp_' nameFld '_' num2str(k) '_*.tiff']);
eval(['!\rm tmp_' nameFld '_' num2str(k) '_*.gif']);

%handle files and dirs
eval(['!mv ' nameFld '_' num2str(k) '.gif ../.']);
cd ..
%rmdir TMP
