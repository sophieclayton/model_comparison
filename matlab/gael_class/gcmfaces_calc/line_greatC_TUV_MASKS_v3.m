...
case 3; lons=[-81 -75]; lats=[28 26]; name='Florida Strait';
case 4; lons=[-81 -79]; lats=[28 22]; name='Florida Strait W1';
case 5; lons=[-76 -76]; lats=[21 8]; name='Florida Strait S1';
case 6; lons=[-75 -75]; lats=[26 24]; name='Florida Strait E1';
case 7; lons=[-75 -75]; lats=[24 21]; name='Florida Strait E2';
...
case 15; lons=[104 109]; lats=[-3 -7]; name='Indonesia W2';
case 16; lons=[113 118]; lats=[-7.5 -7.5]; name='Indonesia W3';
case 17; lons=[118 127 ]; lats=[-7.5 -15]; name='Indonesia W4';
...
case 21; lons=[20 20]; lats=[-30 -72]; name='South Africa Antarctica';
end;
...
if doDisplay;
   ...
   [X,Y,MSK]=convert2pcol(mygrid.XC,mygrid.YC,mygrid.Depth,1,1); MSK(MSK==0)=NaN;
   ...
   [X,Y,mmtIn]=convert2pcol(mygrid.XC,mygrid.YC,line_cur.mmtIn,1,1);
   kk=find(mmtIn==1); plot(X(kk),Y(kk),'m.');

   lon0=median(lons); if lon0>180; lon0=lon0-360; end; lat0=median(lats); 
   axis([lon0-15 lon0+15 lat0-15 lat0+15]);

   %[X,Y,mmtOut]=convert2pcol(mygrid.XC,mygrid.YC,line_cur.mmtOut,1,1);
   %kk=find(mmtOut==1); plot(X(kk),Y(kk),'cx');
   close; close; close;
end;

lons=line_cur.lonPair;
lons(lons>180)=lons(lons>180)-360; 
line_cur.lonPair=lons;
