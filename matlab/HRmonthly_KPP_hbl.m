...
indir='/scratch/jahn/ecco2/cube84/assembled/L1/KPPhbl/';
...
for m=1:12;
    ...
    for day=iter(startday(m)):72:iter(endday(m));
        in=sprintf('%sKPPhbl_day.%010d.data',indir,day);
    ...
    out=sprintf('%sKPP_hbl.monthly.1999%02d.data',outdir,m);
...
out=sprintf('%sKPP_hbl.1999.data',outdir);
fid=fopen(out,'w','ieee-be');
