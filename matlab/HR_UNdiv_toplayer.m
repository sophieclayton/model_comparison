...
% U,V
%[u,v]=HRknit_UVave(it);
u=readbin('/scratch/sclayton/high_res/UVEL_1999.data',[3060 510]);
v=readbin('/scratch/sclayton/high_res/VVEL_1999.data',[3060 510]);
...
fid=fopen('/scratch/sclayton/high_res/Nuts/AveSurfNutrients.1999.data','r','b');
...
[UN,VN]=exch_UV(UN,VN);
for f=1:6;
divUN{f}(:,:,:)=(UN{f}(1:end-1,:,:)-UN{f}(2:end,:,:))./volc{f}(:,:,1);
divVN{f}(:,:,:)=(VN{f}(:,1:end-1,:)-VN{f}(:,2:end,:))./volc{f}(:,:,1);
end
...
out=sprintf('/scratch/sclayton/high_res/horz_flux/divUN_trac%02d_top.data',n);
fidout=fopen(out,'w','b');
fwrite(fidout,divUN,'real*4');
fclose(fidout);

out=sprintf('/scratch/sclayton/high_res/horz_flux/divVN_trac%02d_top.data',n);
fidout=fopen(out,'w','b');
