...
for day=1:365;
    indata=zeros(3060,510,99);
    HRCRnuts=zeros(360,160,99);
    in=sprintf('%sIntTracers.%04d.1999.data',indir,day);
    fid=fopen(in,'r','b');
    indata=fread(fid,3060*510*99,'real*4');
    indata=reshape(indata,[3060 510 99]);
    indata(indata<0)=0;
    fclose(fid);
    
    for tr=1:99;
                
        phi=squeeze(indata(:,:,tr));
        ...
        %end
        % Write out the field
        %         fn=sprintf('/scratch/sclayton/HR2CR_SIarea/%s.%04d.%04d.data',outfpref,day,year);
        %         fid=fopen(fn,'w','ieee-be');
        %         fwrite(fid,fooall,'float32');
        %         fclose(fid);
        
        HRCRnuts(:,:,tr)=fooall;
        clear in phiall fooall 
    end
    
    clear indata
    
    % Write out the field
    fn=sprintf('/scratch/sclayton/HR2CRdata/daily/%s.%04d.1999.data',outfpref,day);
    fid=fopen(fn,'w','ieee-be');
    fwrite(fid,HRCRnuts,'float32');
    fclose(fid);
    day
    clear HRCRnuts
end
